  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <nav className="mb-6 flex items-center gap-2 text-sm text-slate-400">
        <Link href="/providers" className="hover:text-slate-600">Providers</Link>
        <span>/</span>
      ...
      <div className="mb-8 rounded-2xl border border-slate-200 bg-white p-6">
          ...
          <div className="flex-1">
            ...
            {showrooms.length > 0 && (
              <div className="mt-4 flex flex-wrap gap-3">
                {showrooms.map((s) => (
                  <div key={s.id} className="flex items-center gap-2 rounded-lg bg-slate-50 px-3 py-2 text-sm text-slate-600">
            ...
            <div className="flex flex-col gap-2 sm:items-end">
              {primaryShowroom.whatsappNumber && (
                <a
                  ...
                  className="flex items-center gap-2 rounded-xl bg-[#25D366] px-4 py-2.5 text-sm font-semibold text-white hover:bg-[#1ebe5c] transition-colors"
                >
        ...
        <div className="mt-5 flex items-center gap-6 border-t border-slate-100 pt-5 text-sm text-slate-500">
          <span>
            <strong className="font-semibold text-slate-800">{vehicleCount}</strong> vehicle{vehicleCount !== 1 ? 's' : ''}
          </span>
          <span>
            <strong className="font-semibold text-slate-800">{showrooms.length}</strong> showroom{showrooms.length !== 1 ? 's' : ''}
          </span>
        </div>
      </div>
